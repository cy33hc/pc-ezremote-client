...
namespace ZipUtil
{
    ...
    {
        snprintf(activity_message, 1024,"%s %s: %s", lang_strings[STR_EXTRACTING], filename_extracted, fileName);
    }
    ...
    int ZipAddPath(zipFile zf, const std::string &path, int filename_start, int level)
    {
        ...
        if (dfd != NULL)
        {
            ...
            do
            {
                ...
                if (dirent != NULL && strcmp(dirent->d_name, ".") != 0 && strcmp(dirent->d_name, "..") != 0)
                {
                    ...
                    {
                        snprintf(activity_message, 1024,"%s %s", lang_strings[STR_COMPRESSING], new_path);
        ...
        {
            snprintf(activity_message, 1024,"%s %s", lang_strings[STR_COMPRESSING], path.c_str());
     ...
     * Extract a directory.
     */
    static int extract_dir(struct archive *a, struct archive_entry *e, const std::string &path)
    {
        int mode;
        ...
        int ret = archive_read_data_skip(a);
    ...
    static int extract2fd(struct archive *a, const std::string &pathname, int fd)
    {
        ...
        for (int n = 0;; n++)
        {
            ...
            if (len < 0)
            {
                snprintf(status_message, 1024,"error archive_read_data('%s')", pathname.c_str());
                free(buffer);
                return 0;
            }
            ...
            {
                snprintf(status_message, 1024,"error write('%s')", pathname.c_str());
                free(buffer);
                return 0;
            }
        }
    ...
    recheck:
        ...
        if (linkname != NULL)
        {
            ...
            {
                snprintf(status_message, 1024,"error symlink('%s')", path.c_str());
                return;
            }
        ...
        mode_t file_mode = archive_entry_mode(e);
        if ((fd = open(path.c_str(), O_RDWR | O_CREAT | O_TRUNC, file_mode)) < 0)
        {
            snprintf(status_message, 1024,"error open('%s')", path.c_str());
            return;
        }
    ...
    static int extract(struct archive *a, struct archive_entry *e, const std::string &base_dir)
    {
        ...
        if (!FS::MkDirs(realpathname, true))
        {
            free(realpathname);
            free(pathname);
            return 0;
        ...
        if (S_ISDIR(filetype))
        {
            if (!extract_dir(a, e, realpathname))
            {
                free(realpathname);
                free(pathname);
                return 0;
    ...
    {
        Dialog::initImeDialog(lang_strings[STR_PASSWORD], password, 127, 1, 560, 200);
    ...
    int Extract(const DirEntry &file, const std::string &basepath, RemoteClient *client)
    {
        ...
        {
            snprintf(status_message, 1024,"%s", "archive_read_new failed");
            return 0;
        }
        ...
        if (client == nullptr)
        {
            ...
            if (ret < ARCHIVE_OK)
            {
                snprintf(status_message, 1024,"%s", "archive_read_open_filename failed");
                return 0;
            }
        }
        else
        {
            ...
            if (ret < ARCHIVE_OK)
            {
                snprintf(status_message, 1024,"archive_read_set_seek_callback failed - %s", archive_error_string(a));
                return 0;
            }
            ...
            if (ret < ARCHIVE_OK)
            {
                snprintf(status_message, 1024,"%s", "archive_read_open failed");
                return 0;
            }
        }
        ...
        for (;;)
        {
            ...
            if (ret < ARCHIVE_OK)
            {
                snprintf(status_message, 1024,"%s", "archive_read_next_header failed");
                archive_read_free(a);
                return 0;
            }
            ...
            if (!extract(a, e, basepath))
            {
                snprintf(status_message, 1024,"%s", "extract failed");
                archive_read_free(a);
                return 0;
            }
        }
