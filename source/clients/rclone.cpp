    ...
    if (auto res = client->Get(encoded_path))
    {
        ...
        for (size_t i = 1; i < lxb_dom_collection_length(tr_collection); i++)
        {
            ...
            if (date_time.size() > 1)
            {
                ...
                if (adate.size() == 3)
                {
                    gmt.tm_year = atoi(adate[0].c_str())-1900;
                    gmt.tm_mon = atoi(adate[1].c_str());
                    gmt.tm_mday = atoi(adate[2].c_str());
                }

                std::vector<std::string> atime = Util::Split(date_time[1], ":");
                if (atime.size() == 3)
                {
                    gmt.tm_hour = atoi(atime[0].c_str());
                    gmt.tm_min = atoi(atime[1].c_str());

                    std::vector<std::string> sec_msec = Util::Split(atime[2], ".");
                    if (sec_msec.size() > 0)
                    {
                        gmt.tm_sec = atoi(sec_msec[0].c_str());
                    }
                }
            }
            ...
            entry.modified.month = lt.tm_mon;
