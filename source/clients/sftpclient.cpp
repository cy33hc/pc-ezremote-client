...
int SFTPClient::Connect(const std::string &url, const std::string &username, const std::string &password)
{
    ...
    {
        snprintf(ip, 20, "%s", "127.0.0.1");
    }
    else
    {
        if ((he = gethostbyname(host.c_str())) == NULL)
        {
            snprintf(this->response, 512, "%s", lang_strings[STR_COULD_NOT_RESOLVE_HOST]);
            return 0;
        }
    ...
    {
        snprintf(this->response, 512, "%s", "Failed to connect!");
        return 0;
    }
    ...
    if (!session)
    {
        snprintf(this->response, 512, "%s", lang_strings[STR_FAILED]);
        return 0;
    }
    ...
    if (rc)
    {
        snprintf(this->response, 512, "Failed SSL handshake %d", rc);
        return 0;
    }
    ...
    if (auth_pw & 1 && !use_identity)
    {
        ...
        {
            snprintf(this->response, 512, "%s", "Authentication by password failed!");
            goto shutdown;
        }
    }
    else if (auth_pw & 4 && use_identity)
    {
        ...
        {
            snprintf(response, 512, "SSH public key %s is not found", publickey.c_str());
            goto shutdown;
        }
        if (!FS::FileExists(privatekey.c_str()))
        {
            snprintf(response, 512, "SSH private key %s is not found", privatekey.c_str());
            goto shutdown;
        }
        if (libssh2_userauth_publickey_fromfile(session, username.c_str(), publickey.c_str(), privatekey.c_str(), ""))
        {
            snprintf(this->response, 512, "%s", "Authentication by public key failed!");
            goto shutdown;
        }
    }
    else
    {
        snprintf(this->response, 512, "%s", "No supported authentication methods found!");
        goto shutdown;
    }
...
int SFTPClient::Rmdir(const std::string &path, bool recursive)
{
    ...
    for (int i = 0; i < list.size(); i++)
    {
        ...
        if (list[i].isDir && recursive)
        {
            ...
            if (ret == 0)
            {
                snprintf(status_message, 1024, "%s %s", lang_strings[STR_FAIL_DEL_DIR_MSG], list[i].path);
                return 0;
            }
        }
        else
        {
            snprintf(activity_message, 1024, "%s %s\n", lang_strings[STR_DELETING], list[i].path);
            ret = Delete(list[i].path);
            if (ret == 0)
            {
                snprintf(status_message, 1024, "%s %s", lang_strings[STR_FAIL_DEL_FILE_MSG], list[i].path);
                return 0;
            }
        }
    }
...
int SFTPClient::Get(const std::string &outputfile, const std::string &path, uint64_t offset)
{
    ...
    if (!sftp_handle)
    {
        snprintf(response, 512, "Unable to open file with SFTP: %ld", libssh2_sftp_last_error(sftp_session));
        return 0;
    }
    ...
    if (out == NULL)
    {
        snprintf(response, 512, "%s", lang_strings[STR_FAILED]);
        return 0;
    }
...
int SFTPClient::Put(const std::string &inputfile, const std::string &path, uint64_t offset)
{
    ...
    if (!sftp_handle)
    {
        snprintf(response, 512, "%s", "Unable to open file with SFTP");
        return 0;
    }
...
{
    snprintf(this->response, 512, "%s", lang_strings[STR_UNSUPPORTED_OPERATION_MSG]);
    return 0;
}
...
std::vector<DirEntry> SFTPClient::ListDir(const std::string &path)
{
    ...
    do
    {
        ...
        if (rc > 0)
        {
            ...
            snprintf(entry.name, 256, "%s", new_path.c_str());
            ...
            entry.modified.month = tm.tm_mon;
            ...
            entry.st_mode = attrs.permissions;
