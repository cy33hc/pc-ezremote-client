...
int NfsClient::Connect(const std::string &url, const std::string &user, const std::string &pass)
{
	...
	if (nfs == nullptr)
	{
		snprintf(response, 1024, "%s", lang_strings[STR_FAIL_INIT_NFS_CONTEXT]);
		return 0;
	}
	...
	if (nfsurl == nullptr)
	{
		snprintf(response, 1024, "%s", nfs_get_error(nfs));
...
std::vector<DirEntry> NfsClient::ListDir(const std::string &path)
{
	...
	while ((nfsdirent = nfs_readdir(nfs, nfsdir)))
	{
		...
		{
			snprintf(entry.path, 768, "%s%s", path.c_str(), nfsdirent->name);
		}
		else
		{
			snprintf(entry.path, 768, "%s/%s", path.c_str(), nfsdirent->name);
