...
int SmbClient::Connect(const std::string &url, const std::string &user, const std::string &pass)
{
	...
	if (smb2 == NULL)
	{
		snprintf(response, 1024, "Failed to init SMB context");
		return 0;
	}
	...
	{
		snprintf(response, 1024, "Invalid SMB Url");
		return 0;
	}
	...
	{
		snprintf(response, 1024, "%s", smb2_get_error(smb2));
		return 0;
	}
...
int SmbClient::Get(const std::string &outputfile, const std::string &ppath, uint64_t offset)
{
	...
	if (out == NULL)
	{
		snprintf(response, 1024, "%s", lang_strings[STR_FAILED]);
		return 0;
	}
...
{
	snprintf(response, 1024, "%s", lang_strings[STR_UNSUPPORTED_OPERATION_MSG]);
	return 0;
}
...
std::vector<DirEntry> SmbClient::ListDir(const std::string &path)
{
	...
	if (dir == NULL)
	{
		snprintf(status_message, 1024, "%s - %s", lang_strings[STR_FAIL_READ_LOCAL_DIR_MSG], smb2_get_error(smb2));
		return out;
	}
	...
	while ((ent = smb2_readdir(smb2, dir)))
	{
		...
		{
			snprintf(entry.path, 768, "%s%s", path.c_str(), ent->name);
		}
		else
		{
			snprintf(entry.path, 768, "%s/%s", path.c_str(), ent->name);
