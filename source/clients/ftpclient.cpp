...
int FtpClient::Connect(const std::string &url, const std::string &user, const std::string &pass)
{
	...
	else
	{
		...
		{
			snprintf(mp_ftphandle->response, 512, "%s", lang_strings[STR_COULD_NOT_RESOLVE_HOST]);
			return 0;
		}
	...
	if (retval == -1)
	{
		snprintf(mp_ftphandle->response, 512, "%s", lang_strings[STR_FAIL_TIMEOUT_MSG]);
		close(sControl);
		return 0;
	}
	...
	if (!FtpSendCmd(cmd, "3", mp_ftphandle))
	{
		...
		{
			Quit();
			snprintf(mp_ftphandle->response, 512, "%s", lang_strings[STR_FAIL_LOGIN_MSG]);
			return 0;
		}
	}
 ...
 */
const char *FtpClient::LastResponse()
{
...
int FtpClient::FtpAccess(const std::string &path, accesstype type, transfermode mode, ftphandle *nControl, ftphandle **nData)
{
	...
	{
		snprintf(nControl->response, 512, "Missing path argument for file transfer\n");
		return 0;
	}
	snprintf(buf, 512, "TYPE %c", mode);
	...
	case FtpClient::filewrite:
		...
		break;
	default:
		snprintf(nControl->response, 512, "Invalid open type %d\n", type);
		return 0;
	}
	if (path.length() != 0)
	{
...
int FtpClient::FtpOpenPasv(ftphandle *nControl, ftphandle **nData, transfermode mode, int dir, std::string &cmd)
{
	...
	{
		snprintf(nControl->response, 512, "Invalid direction %d\n", dir);
		return -1;
	}
	if ((mode != FtpClient::ascii) && (mode != FtpClient::image))
	{
		snprintf(nControl->response, 512, "Invalid mode %c\n", mode);
		return -1;
	}
	l = sizeof(sin);
	...
	{
		char buf[512];
		snprintf(buf, 512, "REST %lld", mp_ftphandle->offset);
...
int FtpClient::FtpOpenPort(ftphandle *nControl, ftphandle **nData, transfermode mode, int dir, std::string &cmd)
{
	...
	if (getsockname(sData, &sin.sa, &l) < 0)
		return 0;
	snprintf(buf, 512, "PORT %hhu,%hhu,%hhu,%hhu,%hhu,%hhu",
...
int FtpClient::Size(const std::string &path, int64_t *size)
{
	...
	snprintf(cmd, 512, "TYPE %c", FtpClient::transfermode::image);
	if (!FtpSendCmd(cmd, "2", mp_ftphandle))
		return 0;

	snprintf(cmd, 512, "SIZE %s", path.c_str());
	if (!FtpSendCmd(cmd, "2", mp_ftphandle))
		rv = 0;
	else
	{
...
std::vector<DirEntry> FtpClient::ListDir(const std::string &path)
{
	...
	if (nData != NULL)
	{
		...
		while (ret > 0)
		{
			...
			{
				if (!show_hidden_files && entry.name[0] == '.')
					continue;
...
{
	snprintf(mp_ftphandle->response, 512, "%s", lang_strings[STR_UNSUPPORTED_OPERATION_MSG]);
	return 0;
}
