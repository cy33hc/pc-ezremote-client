...
void WriteString(cchr *Section, cchr *pKey, cchr *Value)
{
	...
	{
		snprintf(Str, 512, "%s=%s%s", List.KeyText, Value, List.Comment);
...
{
	double Val;
	snprintf(Result, 520, "%1.10lE", Default);
...
{
	char Text[512];
	snprintf(Text, 512, "[%s]", Section);
	if (AddItem(tpSECTION, Text) == FALSE)
	{
		return FALSE;
	}
	snprintf(Text, 512, "%s=%s", pKey, Value);
