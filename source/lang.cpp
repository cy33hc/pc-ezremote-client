...
char lang_strings[LANG_STRINGS_NUM][LANG_STR_SIZE] = {
	...
	"Failed to create folder",                                                                        // STR_FAIL_CREATE_LOCAL_FOLDER_MSG
	...
	"Other",                                                                                          // STR_OTHER
	"Update",                                                                                         // STR_UPDATE
	"Failed to update permission",                                                                    // STR_FAIL_UPDATE_PERMISSION_MSG
};
...
namespace Lang
{
	void SetTranslation()
	{
		...
		{
			snprintf(langFile, 768, "%s/share/ezremote-client/assets/langs/%s.ini", g_root_path, lang.c_str());
		}
		else
		{
			snprintf(langFile, 768, "%s/share/ezremote-client/assets/langs/English.ini", g_root_path);
		}
		...
		snprintf(display_site, 32, "%s %d", lang_strings[STR_SITE], num);
	}
}